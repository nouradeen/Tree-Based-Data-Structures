...
class TerminalUI:
    ...
    def show_2d(self):
        ...
        '''
        tree_list = self._tree.bfs_order_star()
        start = 0
        end = 1
        line = 2

        for i in range(self._tree.height()):
            empty_space = (int)(self.menu_width() / (line))
            
            for j in range(start, end):
                print(f"\t{tree_list[j] if tree_list[j] is not None else '*'}".expandtabs(empty_space), end="")
                print("\t".expandtabs(empty_space), end="")
            print("\n")
